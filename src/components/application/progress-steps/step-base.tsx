...
export const statuses = {
    ...
    current: {
        icon: "bg-brand-secondary-600 ring-2 ring-focus-ring ring-offset-bg-primary ring-offset-2",
        dot: "bg-fg-white",
        connector: "border-secondary",
        title: "text-brand-secondary-secondary",
        description: "text-brand-secondary-tertiary",
    },
    complete: {
        icon: "bg-brand-secondary-solid",
        dot: "hidden",
        connector: "border-brand-secondary",
        title: "text-secondary",
        description: "text-tertiary",
    },
};
    ...
    return (
        <div className={cx("flex w-full flex-col items-center justify-center gap-4", size === "sm" && "gap-3")}>
            <div className={cx("relative flex w-full flex-col items-center self-stretch")}>
                {status === "complete" ? (
                    <span className={cx("z-10 flex items-center justify-center rounded-full bg-success-solid", size === "sm" ? "size-10" : "size-12")}>
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className={cx(size === "sm" ? "size-3" : "size-4")}>
                            <path d="M10 3L4.5 8.5L2 6" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                    </span>
                ...
                {connector && (
                    <span
                        className={cx(
                            ...
                            status === "complete" ? "border-success-solid" : "border-secondary",
                        )}
                    />
                )}
            </div>
...
export const TextLine = <T extends ComponentType>({ status = "incomplete", size = "sm", title, description }: StepBaseProps<T>) => {
    return (
        <div className={cx("relative flex w-full flex-col items-center justify-center pt-2", size === "sm" && "pt-3", size === "md" && "pt-4")}>
            <div className={cx("absolute inset-x-0 top-0 h-1 bg-brand-secondary-solid", status === "incomplete" && "bg-quaternary")} />

            <div className={cx("flex w-full flex-col items-start gap-0.5 self-stretch", size === "sm" && "gap-0")}>
                <p
                    className={cx(
                        "text-secondary",
                        status === "current" && "text-brand-secondary-secondary",
                        size === "sm" ? "text-sm font-semibold" : "text-md font-semibold",
                    )}
                >
                    {title}
                </p>
                <p className={cx("text-tertiary", status === "current" && "text-brand-secondary-tertiary", size === "sm" ? "text-sm" : "text-md")}>{description}</p>
            </div>
        </div>
    );
};
