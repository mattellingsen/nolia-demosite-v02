...
async function assessFileAgainstFund(file: File, fund: any): Promise<UIAssessmentResult> {
    ...
    try {
        ...
        // Call the resilient assessment API (V2)
        formData.append('fundId', fund.id);
        const endpoint = '/api/process/test-assessment-v2';
        ...
        if (data.success && data.assessment) {
            ...
            if (assessment.transparencyInfo) {
                result.transparencyInfo = assessment.transparencyInfo;
            }

            return result;
        } else if (data.score !== undefined) {
            // V2 legacy assessment response
            const assessment = {
                overallScore: data.score,
                scores: {
                    innovation: data.innovation || data.score,
                    financial: data.financial || data.score,
                    team: data.team || data.score,
                    market: data.market || data.score
                },
                feedback: data.feedback || 'Assessment completed',
                strengths: data.strengths || [],
                weaknesses: data.weaknesses || [],
                formattedOutput: data.formattedOutput || '',
                strategyUsed: data.strategyUsed,
                transparencyInfo: data.transparencyInfo
            };

            const result = convertToUIResult(file, assessment as any, fund);

            // Add transparency information
            if (data.transparencyInfo) {
                result.transparencyInfo = data.transparencyInfo;
                result.summary = `${result.summary} • ${data.transparencyInfo.userMessage}`;
            }
