...
export const Step1UploadForm: React.FC<Step1Props> = ({ 
    ...
    const analyzeForm = async (file: File) => {
        ...
        // Simulate AI analysis of the uploaded form
        try {
            await new Promise(resolve => setTimeout(resolve, 2000)); // Simulate processing time
            
            // Mock analysis results based on file name or type
            const mockAnalysis: FormAnalysis = {
                questionsFound: Math.floor(Math.random() * 15) + 5, // 5-20 questions
                fieldTypes: ['Text Input', 'Multiple Choice', 'File Upload', 'Date Picker', 'Number Input'],
                sections: ['Business Information', 'Project Details', 'Financial Information', 'Supporting Documents'],
                complexity: file.name.includes('complex') ? 'Complex' : file.name.includes('simple') ? 'Simple' : 'Medium'
            };
            
            setAnalysis(mockAnalysis);
        } catch (error) {
            setUploadError('Failed to analyze the form. Please try again.');
        } finally {
            ...
            {analysis && formData.applicationForm && (
                <div className="bg-gray-50 rounded-lg p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <p className="text-sm font-medium text-primary mb-2">Detected Sections:</p>
                        ...
                        <div>
                            <p className="text-sm font-medium text-primary mb-2">Field Types Found:</p>
                            <div className="flex flex-wrap gap-2">
