...
// Function to assess a file against tender criteria using real AI assessment
async function assessFileAgainstTender(file: File, tender: any): Promise<UIAssessmentResult> {
    if (!tender?.id) {
        throw new Error('Tender ID is required for assessment');
    }
    ...
    try {
        ...
        const response = await fetch(`/api/assess/${tender.id}`, {
            method: 'POST',
            body: formData,
        });
        ...
        return convertToUIResult(file, assessment, tender);
        ...
        return {
            fileName: file.name,
            rating: 0,
            categories: [tender?.name || 'Unknown Tender'],
...
interface AssessmentProcessorProps {
    files: File[];
    selectedTender: any; // Tender from useTenders hook
