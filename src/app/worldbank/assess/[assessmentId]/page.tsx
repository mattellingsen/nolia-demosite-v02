...
export default function AssessmentDetailPage() {
  ...
  const { data: response, isLoading, error } = useQuery<AssessmentResponse>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/worldbank-assessments/${assessmentId}`);
      if (!res.ok) {
      ...
      <div className="flex flex-col bg-primary lg:flex-row">
        <SidebarNavigationSlim
          ...
          items={[
            ...
            { label: "Analytics", href: "/worldbank/analytics", icon: TrendUp02 },
          ]}
        />
        ...
        <main className="flex min-w-0 flex-1 flex-col gap-8 pt-8 pb-12 px-4 lg:px-8">
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <Button onClick={() => router.push('/worldbank/assess')} size="md" color="primary">
                Back to Assessments
              </Button>
            </div>
          </div>
    ...
    <div className="flex flex-col bg-primary lg:flex-row">
      ...
      <main className="flex min-w-0 flex-1 flex-col gap-8 pt-8 pb-12 px-4 lg:px-8">
        ...
        <div className="flex flex-col gap-4">
          <Button
            onClick={() => router.push('/worldbank/assess')}
            size="sm"
            color="tertiary"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-secondary rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-sm font-medium text-primary">Project</h3>
            </div>
            <p className="text-md font-medium text-primary">{assessment.project.name}</p>
          </div>
          ...
          <div className="bg-secondary rounded-2xl p-6">
            ...
            <div className="space-y-4">
              {assessment.project.description && (
                <div>
                  <h3 className="text-sm font-medium text-primary mb-1">Project Description</h3>
                  <p className="text-sm text-tertiary">{assessment.project.description}</p>
                </div>
              )}
