...
import { useProcurementAssessments } from "@/hooks/useProcurementAssessments";
import {
    ArrowRight,
    CheckDone01,
    Edit01,
...
const tenderCards = [
    {
        id: 1,
        title: "IT Services",
        description: "Procurement of comprehensive IT services and infrastructure solutions.",
        gradient: "from-[#A5C0EE] to-[#FBC5EC]"
    },
    {
        id: 2,
        title: "Professional Services",
        description: "Tender for professional consulting and advisory services.",
        gradient: "from-[#FBC2EB] to-[#A18CD1]"
    }
];

export const ProcurementApplyDashboard = () => {
    const [mounted, setMounted] = useState(false);

    // Fetch tenders data for the right sidebar
    ...
    const assessmentsQuery = useProcurementAssessments();
    ...
    const activeFormsCount = 0; // Forms created, not tenders
    const submissionsCount = assessmentsQuery.data?.pagination?.total || 0; // Real submissions count from assessments
            ...
            <main className="flex min-w-0 flex-1 flex-col gap-8 pt-8 pb-12">
                <div className="flex flex-col flex-wrap items-start justify-between gap-x-4 gap-y-5 px-4 lg:flex-row lg:px-8">
                    <div className="flex flex-col gap-1">
                        <p className="text-md font-semibold text-tertiary">Submission Forms</p>
                        <p className="text-display-md font-semibold text-primary">Form Management</p>
                    </div>
                    <div className="flex flex-col-reverse gap-3 sm:flex-row">
                        <DateRangePicker />
                        <Button color="primary" size="md" iconLeading={Plus} href="/procurement/apply/setup-form">
                            Setup form
                        </Button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-5 px-4 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
                    ...
                    <div className="flex flex-col gap-2 rounded-xl bg-primary p-4 shadow-xs ring-1 ring-secondary ring-inset">
                        <p className="text-sm font-medium text-tertiary">Submissions This Month</p>
                        <div className="flex items-baseline gap-2">
                            <p className="text-display-sm font-semibold text-primary">{submissionsCount}</p>
                        </div>
                    </div>
                    <div className="flex flex-col gap-2 rounded-xl bg-primary p-4 shadow-xs ring-1 ring-secondary ring-inset">
                ...
                <div className="px-4 lg:px-8">
                    <div className="flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            <p className="text-lg font-semibold text-primary">Submission Forms</p>
                        </div>

                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {/* Create new submission form card */}
                            <a href="/procurement/apply/setup-form" className="w-full h-40 relative flex cursor-pointer hover:opacity-90 transition-opacity">
            ...
            <div className="sticky top-0 hidden h-screen w-98 flex-col gap-8 overflow-auto border-l border-secondary bg-primary pb-12 lg:flex">
                <div className="flex shrink-0 flex-col gap-5 overflow-x-clip px-6 pt-8">
                    <div className="flex justify-between">
                        ...
                        <Button size="md" color="link-gray" iconLeading={Plus} href="/procurement/setup">
                            Add tender
                        </Button>
                    </div>
                    {mounted && (
                    <Carousel.Root className="flex flex-col gap-5">
                        <Carousel.Content overflowHidden={false} className="gap-5">
                            {activeTenders.map((tender, index) => {
                                ...
                                return (
                                    <Carousel.Item key={tender.id} className="basis-auto">
                                        <div className="w-68 h-40 relative flex">
                                            <div className={`w-full h-full flex flex-col justify-between overflow-hidden rounded-2xl p-4 bg-linear-to-b ${gradient} before:pointer-events-none before:absolute before:inset-0 before:z-1 before:rounded-[inherit] before:mask-linear-135 before:mask-linear-to-white/20 before:ring-1 before:ring-white/30 before:ring-inset`}>
                                                ...
                                                <div className="relative flex items-center justify-between px-1 pt-1">
                                                    <div className="text-md leading-[normal] font-semibold text-white">{tender.name}</div>
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex flex-col gap-2">
                                <div className="flex justify-between gap-4">
                                    ...
                                    <span className="text-sm text-tertiary">{submissionsCount} Submissions</span>
                                </div>
                                <ProgressBar value={0} />
                            </div>
                            {activeTenders.length > 0 && <CarouselIndicator size="lg" framed={false} />}
                        </div>
                    </Carousel.Root>
                    )}
                </div>
