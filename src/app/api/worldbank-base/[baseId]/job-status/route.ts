...
export async function GET(
  ...
  try {
    ...
    // Convert to a more usable format for worldbank bases
    ...
    if (ragJob && ragJob.status === 'PENDING') {
      ...
      if (jobAge > 5000) {
        ...
        try {
          ...
          const assemblyUrl = `${baseUrl}/api/worldbank-brain/${baseId}/assemble`;
    ...
    console.error('Error fetching worldbank base job status:', error);
    return NextResponse.json(
      {
