...
export async function GET(
  ...
  try {
    ...
    // Get all jobs for this procurement tender
    ...
    // Convert to a more usable format for procurement tenders (4-step structure)
    ...
    if (!tender) {
      return NextResponse.json(
        { error: 'Procurement tender not found' },
        { status: 404 }
      );
    }
    ...
    if (ragJob && ragJob.status === 'PENDING') {
      ...
      if (jobAge > 5000) {
        ...
        try {
          ...
          const assemblyUrl = `${baseUrl}/api/procurement-brain/${tenderId}/assemble`;
          ...
          fetch(assemblyUrl, {
            ...
            if (response.ok) {
              console.log(`✅ Successfully triggered brain assembly for tender ${tenderId}`);
            } else {
    ...
    if (!ragJob && tender?.fundBrain) {
      ragJob = {
        ...
        errorMessage: null,
        startedAt: tender.brainAssembledAt,
        completedAt: tender.brainAssembledAt,
        createdAt: tender.brainAssembledAt,
        updatedAt: tender.brainAssembledAt,
    ...
    console.error('Error fetching procurement tender job status:', error);
    return NextResponse.json(
      {
...
function getCurrentTask(ragJob: any): string {
  ...
  if (ragJob.status === 'COMPLETED') {
    return 'Tender knowledgebase complete';
  }
  ...
  // Map progress to tasks (tender-specific messaging)
  ...
  if (progress < 95) return 'Building tender knowledgebase...';
