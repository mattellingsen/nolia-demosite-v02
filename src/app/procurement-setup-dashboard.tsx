...
// Generate gradient colors for tenders
...
export const ProcurementSetupDashboard = () => {
    ...
    const handleDeleteTender = async (tenderId: string, event?: React.MouseEvent) => {
    ...
    const handleTenderCardClick = (tenderId: string) => {
        router.push(`/procurement/tender-created?tenderId=${tenderId}`);
    };
            ...
            <main className="flex min-w-0 flex-1 flex-col gap-8 pt-8 pb-12">
                <div className="flex flex-col flex-wrap items-center justify-center gap-x-4 gap-y-5 px-4 lg:flex-row lg:px-8">
                    <div className="flex flex-col gap-1 text-center">
                        ...
                        <p className="text-display-md font-semibold text-primary">Tender Admin & Setup</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-5 px-4 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
                    <div className="flex flex-col gap-2 rounded-xl bg-primary p-4 shadow-xs ring-1 ring-secondary ring-inset">
                        <p className="text-sm font-medium text-tertiary">Active Tenders</p>
                        <div className="flex items-baseline gap-2">
                            <p className="text-display-sm font-semibold text-primary">
                                {tendersLoading ? '...' : tenders.filter(t => t.status === 'ACTIVE').length}
                            </p>
                        </div>
                    </div>
                    <div className="flex flex-col gap-2 rounded-xl bg-primary p-4 shadow-xs ring-1 ring-secondary ring-inset">
                        <p className="text-sm font-medium text-tertiary">Total Tenders</p>
                        <div className="flex items-baseline gap-2">
                            <p className="text-display-sm font-semibold text-primary">
                    ...
                    <div className="flex flex-col gap-2 rounded-xl bg-primary p-4 shadow-xs ring-1 ring-secondary ring-inset">
                        <p className="text-sm font-medium text-tertiary">Draft Tenders</p>
                        <div className="flex items-baseline gap-2">
                            <p className="text-display-sm font-semibold text-primary">
                                {tendersLoading ? '...' : tenders.filter(t => t.status === 'DRAFT').length}
                            </p>
                        </div>
                    </div>
                    <div className="flex flex-col gap-2 rounded-xl bg-primary p-4 shadow-xs ring-1 ring-secondary ring-inset">
                        ...
                        <div className="flex items-baseline gap-2">
                            <p className="text-display-sm font-semibold text-primary">
                                {tendersLoading ? '...' : tenders.filter(t => {
                ...
                <div className="px-4 lg:px-8">
                    <div className="flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            <p className="text-lg font-semibold text-primary">Tender Setup</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            {tendersLoading ? (
                                ...
                                tenders.map((tender, index) => (
                                    <div key={tender.id} className="w-full h-50 relative flex cursor-pointer hover:opacity-90 transition-opacity" onClick={() => handleTenderCardClick(tender.id)}>
                                        <div className={`w-full h-full relative overflow-hidden rounded-2xl p-4 bg-linear-to-b ${generateGradient(index)} before:pointer-events-none before:absolute before:inset-0 before:z-1 before:rounded-[inherit] before:mask-linear-135 before:mask-linear-to-white/20 before:ring-1 before:ring-white/30 before:ring-inset`}>
                                            ...
                                            <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3">
                                                <div className="flex min-w-0 flex-col">
                                                    <div className="text-md leading-[normal] font-semibold text-white mb-3">{tender.name}</div>
                                                    <p className="text-xs leading-snug font-light text-white mb-5" style={{wordBreak: "break-word"}}>
                                                        {tender.description || 'No description provided'}
                                                    </p>
                                                    <div className="flex items-center gap-2">
                                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                            tender.status === 'ACTIVE' ? 'bg-green-500/20 text-green-200' :
                                                            tender.status === 'DRAFT' ? 'bg-yellow-500/20 text-yellow-200' :
                                                            'bg-gray-500/20 text-gray-200'
                                                        }`}>
                                                            {tender.status}
                                                        </span>
                                                        <span className="text-xs text-white/70">
                                                            {formatDate(new Date(tender.createdAt).getTime())}
                                                        </span>
                                                    </div>
                                                </div>
                                                <ButtonUtility
                                                    ...
                                                    onClick={(event) => handleDeleteTender(tender.id, event)}
                                                    isDisabled={deleteTender.isPending}
                                                />
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {/* Setup new tender card */}
                            <a href="/procurement/setup/setup-new-tender" className="w-full h-50 relative flex cursor-pointer hover:opacity-90 transition-opacity">
